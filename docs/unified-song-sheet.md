...
Production uses Fly secrets `CHORDLYZE_WORKER_TOKEN` and `APIFY_TOKEN`. The latter should be a dedicated scoped token allowed to run the two named Actors and access their run output. `backend/fly.toml` configures the provider, limits and process groups. The Docker image contains both the API and worker and installs the pinned model. Secrets are excluded from Git and Docker.
