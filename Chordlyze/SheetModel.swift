...
/// Only analyzed chords are placed: lines past the analyzed audio get none,
/// and a preview excerpt (unknown offset in the song) places nothing at all.
...
enum SheetModel {
    ...
    static func build(analysis: ChordAnalysis, lines: [LyricLine]) -> [RenderLine] {
        let real: [(start: Double, chord: Chord)] = analysis.isPreview ? []
            : analysis.chords.compactMap { seg in seg.chord.map { (seg.start, $0) } }
        let coverageEnd = analysis.isPreview ? 0 : analysis.coverageEnd

        var result: [RenderLine] = []
        for (index, line) in lines.enumerated() {
            let gapEnd = index + 1 < lines.count ? lines[index + 1].time : line.time + 6
            guard line.time < coverageEnd, !real.isEmpty else {
                result.append(RenderLine(id: line.time, end: gapEnd, text: line.text, chords: []))
                continue
            }
