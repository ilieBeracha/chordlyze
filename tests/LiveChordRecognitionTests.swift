...
@main struct LiveChordRecognitionTests {
    private static func sensitivityTests(mode: ChordDrillDetector.Mode) throws {
        for sr in [44100.0, 48000.0] {
            ...
            for (label, notes) in [("Am", [45,52,57,60,64]), ("C", [48,52,55,60,64]), ("G", [43,47,50,55,59,67])] {
                for decay in [0.0, 3.0] {
                    ...
                    for level: Float in [0.002, 0.001, 0.0005] {
                        let frames = feed(try ChordDrillDetector(sampleRate: sr, mode: mode), scaled(raw, rms: level))
            ...
            for rms: Float in [0.00005, 0.0002, 0.002, 0.02] {
                let frames = feed(try ChordDrillDetector(sampleRate: sr, mode: mode), noise.map { $0*rms })
            ...
            let detector = try ChordDrillDetector(sampleRate: sr, mode: mode)
            ...
            check(history(feed(try ChordDrillDetector(sampleRate: sr, mode: mode), stopped)).isEmpty,
            ...
            check(history(feed(try ChordDrillDetector(sampleRate: sr, mode: mode), single)).isEmpty,
    ...
    static func main() throws {
        ...
        for mode in [ChordDrillDetector.Mode.liveRecognition, .practiceFeedback] {
            for sr in [44100.0, 48000.0] {
                ...
                let extensions = try ChordDrillDetector(sampleRate: sr, mode: mode)
            ...
            try sensitivityTests(mode: mode)
        }
        // The default uses the stable song-practice policy.
        let defaultPolicy = try ChordDrillDetector(sampleRate: 44100)
        ...
        let a = feed(defaultPolicy,audio), b = feed(explicit,audio)
        check(history(a) == history(b), "Default selects stable practice behavior")
        check(a.map(\.recognizedAt) == b.map(\.recognizedAt), "Default and explicit practice acceptance timestamps agree")
